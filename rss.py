...
HOST = '0.0.0.0'
PORT = 5002  # Different port for red snake
...
# Game state
current_direction = "Left"  # Starting direction for red snake
red_immunity = 0
immunity_period = 6
apple_pos = None
red_head_pos = None
...
def decide_move(red_pos, yellow_pos, apple_pos, has_immunity=False):
    ...
    if first_move:
        ...
        edge_safe_dir = check_edge_proximity(red_grid)
        if edge_safe_dir:
    ...
    if has_immunity:
        ...
        path = a_star_search(red_grid, yellow_grid, has_immunity=True)
        if path and len(path) > 0:
            next_pos = path[0]
            for direction, (dx, dy) in directions.items():
                new_pos = (red_grid[0] + dx, red_grid[1] + dy)
    ...
    if apple_grid:
        ...
        path = a_star_search(red_grid, apple_grid, has_immunity)
        ...
        if path and len(path) > 0:
            ...
            for direction, (dx, dy) in directions.items():
                ...
                if new_pos == next_pos and direction in safe_directions:
                    ...
                    if not will_hit_wall(red_grid, direction):
                        return direction
        else:
    ...
    if current_direction in safe_directions and not will_hit_wall(red_grid, current_direction):
...
def update_opponent_positions(yellow_pos):
    """Update the list of opponent positions."""
    global opponent_positions, yellow_length
    
    yellow_grid_pos = get_grid_position(yellow_pos)
    
    # Only add if it's a new position
    if not opponent_positions or yellow_grid_pos != opponent_positions[0]:
        # Add new position to front of list
        opponent_positions.insert(0, yellow_grid_pos)
        
        # Keep list length equal to opponent's length
        # while len(opponent_positions) > yellow_length:
...
def process_game_state(data):
    ...
    global current_direction, red_immunity, apple_pos, red_head_pos, yellow_head_pos
    global red_length, yellow_length, opponent_positions
    ...
    try:
        ...
        red_grid = get_grid_position(red_center)
        edge_safe_dir = check_edge_proximity(red_grid)
        if edge_safe_dir:
        ...
        if check_apple_eaten(current_apple_pos, apple_pos):
            ...
            if red_head_pos and manhattan_distance(red_head_pos, apple_pos) < SEG_SIZE:
                red_length += 1
                red_immunity = immunity_period
                print(f"We ate an apple! Length: {red_length}, Immunity: {red_immunity}")
            # If the apple was at opponent's position, they ate it
            elif yellow_head_pos and manhattan_distance(yellow_head_pos, apple_pos) < SEG_SIZE:
                yellow_length += 1
                print(f"Opponent ate an apple! Their length: {yellow_length}")
        
        # Update position tracking
        update_opponent_positions(yellow_center)
        ...
        # Decide next move
        next_move = decide_move(red_center, yellow_center, current_apple_pos, has_immunity=(red_immunity > 0))
        
        # Final safety check - don't let snake hit a wall
        if will_hit_wall(red_grid, next_move):
            print(f"WARNING: {next_move} would hit wall, finding alternative")
            safe_dirs = get_safe_directions(red_grid, red_immunity > 0)
            if safe_dirs:
